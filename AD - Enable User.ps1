...
# New Service
# AD - Enable User Account
# v 1.0 Initial Release
# Init Release: 03.02.2020
# Last Update: 03.02.2020
# Code Template V 1.1
# URL: https://au2mator.com/enable-user-account-active-directory-self-service-with-au2mator/
...
[string]$LogfileName = "Enable User Account"
...
try {
    ...
    Get-ADUser -Identity $c_User  | Set-ADAccountPassword -NewPassword $NewPassword -Reset -PassThru | Set-ADUser -ChangePasswordAtLogon $true -PassThru | Enable-ADAccount 
...
if ($ErrorCount -eq 0) {
    ...
    $AdditionalHTML = "<br>
        USer " + (Get-ADUser -identity $c_User).DisplayName + " has been enabled and Password was reset
        <br>
        "
    $Status = "COMPLETED"
}
else {
    $au2matorReturn = "failed to enable User $c_User, Error: $Error"
    $Status = "ERROR"
}
