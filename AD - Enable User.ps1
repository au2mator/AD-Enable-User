...
# Type: New Service
#
# Title: AD - Enable User Account
#
# v 1.0 Initial Release
# v 1.1 Added Stored Credentials
#       see for details: https://au2mator.com/documentation/powershell-credentials/?utm_source=github&utm_medium=social&utm_campaign=AD_EnableUser&utm_content=PS1
...
# Code Template V 1.3
#
# URL: https://au2mator.com/enable-user-account-active-directory-self-service-with-au2mator/?utm_source=github&utm_medium=social&utm_campaign=AD_EnableUser&utm_content=PS1
# Github: https://github.com/au2mator/AD-Enable-User
#################
...
param (
    ...
    [parameter(Mandatory = $false)] 
    [String]$c_CheckPassword,

    [parameter(Mandatory = $false)] 
    [String]$c_customPassword,
...
[string]$LogPath = "C:\_SCOworkingDir\TFS\PS-Services\AD - Enable User"
[string]$LogfileName = "Enable User"

[string]$CredentialStorePath = "C:\_SCOworkingDir\TFS\PS-Services\CredentialStore" #see for details: https://au2mator.com/documentation/powershell-credentials/?utm_source=github&utm_medium=social&utm_campaign=AD_EnableUser&utm_content=PS1
...
# Stored Credentials
# See: https://au2mator.com/documentation/powershell-credentials/?utm_source=github&utm_medium=social&utm_campaign=AD_EnableUser&utm_content=PS1
...
Write-au2matorLog -Type INFO -Text "Try to enable AD User and reset Password"

try {
    if ($c_CheckPassword -eq "True")
    {
        $NewPassword=(ConvertTo-SecureString -AsPlainText "$c_customPassword" -Force)
        Write-au2matorLog -Type INFO -Text "using Custom Password"
    }
    else
    {
        $NewPassword=(ConvertTo-SecureString -AsPlainText "!PAssword-1Password-1" -Force)
        Write-au2matorLog -Type INFO -Text "using standard Password"
    }

    Set-ADAccountPassword -Identity $C_User -NewPassword $NewPassword -Reset -ErrorAction stop
    Set-ADUser -Identity $C_User -ChangePasswordAtLogon $true
    Enable-ADAccount -Identity $c_User -ErrorAction stop

    Write-au2matorLog -Type INFO -Text "User enabled and Password set"

    $au2matorReturn = "User $((Get-ADUser -identity $c_User).Name) has been enabled and Password is set"
    $AdditionalHTML = "<br>
        User $((Get-ADUser -identity $c_User).Name) has been enabled and Password is set
        <br>
        "
    $Status = "COMPLETED"
}
    ...
    Write-au2matorLog -Type ERROR -Text "Not able to enable User and reset Password"
    Write-au2matorLog -Type ERROR -Text $Error

    $au2matorReturn = "failed to enable User $((Get-ADUser -identity $c_User).Name), Error: $Error"
    $AdditionalHTML = "<br>
    failed to enable User $((Get-ADUser -identity $c_User).Name)
    <br>
    Error: $Error
    "
    $Status = "ERROR"
